...
public class JwtAuthHttpClient : IAuthService
{
    ...
    public async Task LoginAsync(string username, string password)
    {
        ...
        StringContent content = new(userAsJson, Encoding.UTF8, "application/json");
         ...
         HttpResponseMessage response = await client.PostAsync("http://localhost:5098/auth/login", content);
       // HttpResponseMessage response = await client.PostAsJsonAsync("http://localhost:5098/auth", content);
